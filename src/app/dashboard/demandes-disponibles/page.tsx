...
export default async function DemandesDisponiblesPage() {
  ...
  const [profileResult, demandesResult, specialitesResult] = await Promise.all([
    ...
    supabase.from('demandes').select(`id, titre, region, secteur, date_debut, statut, specialite_id, profiles ( full_name )`).neq('statut', 'ferme').order('created_at', { ascending: false }),
    supabase.from('specialites').select('id, nom').order('nom'),
  ])

  const plan = profileResult.data?.plan ?? null
  const isPro = plan === 'pro_trial' || plan === 'pro' || plan === 'entreprise'
  ...
  return (
    <div style={{ maxWidth: '1100px' }}>
      <DemandesDisponibles demandes={demandes} specialites={specialitesResult.data ?? []} isPro={isPro} />
    </div>
  )
}
